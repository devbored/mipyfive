...
def test_regfile_read():
    def test(self):
        ...
        def process():
            ...
            if useBRAM:
                for i in range(self.dut.dualPortBram.memory.depth):
                    randVal = random.randint(1, 4294967295)
                    yield self.dut.dualPortBram.memory[i].eq(randVal)
...
def test_regfile_write(writeData):
    def test(self):
        ...
        def process():
            ...
            if useBRAM:
                for i in range(self.dut.dualPortBram.memory.depth):
                    yield self.dut.dualPortBram.memory[i].eq(random.randint(1, 4294967295))
                yield Tick()
...
class TestRegfile(unittest.TestCase):
    def setUp(self):
        self.dut = RegFile(width=32, regCount=32, bramRegfile=useBRAM)
...
if __name__ == "__main__":
    ...
    if args.bram is True:
        print(f"[mipyfive - Info]: Testing using BRAM implementation.")
        useBRAM = True
    else:
        print(f"[mipyfive - Info]: Testing using LUT implementation.")
